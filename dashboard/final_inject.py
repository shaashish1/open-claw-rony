#!/usr/bin/env python3
"""Final inject: read clean_ops.html, inject cards, write to ops.html"""
import os

SRC = '/tmp/clean_ops.html'
...
PROJECTS = [
  ['IT','ITGYANI Agency','active','high','AI automation agency - first 5 clients, service page, cold outreach','https://itgyani.com',J+'/jira/software/projects/IT/boards/7',41,0,0,'RONY/ZARA','Rs 0 to Rs 1L/mo',[['IT-15','First 5 clients'],['IT-20','Service page live'],['IT-21','100 cold emails']]],
  ...
  ['YUKTI','Yukti Algo Trading','active','high','OpenAlgo algo trading SaaS - 10 strategies in analyze mode','https://openalgo.cryptogyani.com',J+'/jira/software/projects/YUKTI/boards',7,0,0,'KABIR/VIKRAM','Rs 1Cr paper to SaaS beta',[['YUKTI-3','VPS audit'],['YUKTI-5','Trading dashboard'],['YUKTI-6','P&L integration']]],
...
for a in AGENTS:
    ...
    anim = 'animation:pulse 2s infinite' if status in ('live','build') else ''
    ...
    agent_html += (
        ...
        f'<span style="width:7px;height:7px;border-radius:50%;display:inline-block;background:{dot_col};{anim};flex-shrink:0"></span>'
...
# Inject cards
html = html.replace('<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3" id="agents-grid"></div>',
                    f'<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3" id="agents-grid">\n{agent_html}</div>')
html = html.replace('<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3" id="projects-grid"></div>',
                    f'<div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3" id="projects-grid">\n{proj_html}</div>')
...
checks = {
    'Single switchTab': content.count('function switchTab') == 1,
    'No old tab()': content.count('function tab(') == 0,
    'Single </html>': content.count('</html>') == 1,
    'Agents (RONY/KABIR)': 'RONY' in content and 'KABIR' in content,
    'Projects (CryptoGyani)': 'CryptoGyani' in content,
    '15-min countdown': 'countdown=15' in content,
    'Jira IT-34 link': 'IT-34' in content,
}
